"""
Data fetching: BaoStock primary (A-share dedicated), yfinance fallback.
"""
...
def _retry_download_baostock(ticker: str, start_date: str, end_date: str, retries=3):
    """Use BaoStock to fetch daily data."""
...
def _to_baostock_ticker(ticker: str) -> str:
    """Convert clean 6-digit code to baostock format."""
    ...
    if not ticker:
        return ticker
    # Shanghai: 5 or 6, Shenzhen: 0, 2, 3
    if ticker[0] in ('5', '6'):
        return f"sh.{ticker}"
    else:
        return f"sz.{ticker}"
...
def _to_yfinance_ticker(ticker: str) -> str:
    """Convert to yfinance format as fallback."""
    ticker = str(ticker).strip()
    for suffix in ['.SH', '.SZ', '.SS']:
        if ticker.upper().endswith(suffix):
            ticker = ticker[:-len(suffix)]
...
def get_stock_hist(ticker: str, start_date: str = "2020-01-01", end_date: str = "2050-01-01") -> pd.DataFrame:
    """Fetch daily OHLCV data: try BaoStock first, then yfinance."""
    # Try BaoStock
    ...
    try:
        ...
        st.warning(f"Fallback yfinance failed for {ticker}: {e}")

    st.warning(f"No data found for {ticker} (tried BaoStock and yfinance)")
    return pd.DataFrame()


def get_etf_hist(ticker: str, start_date: str = "2020-01-01", end_date: str = "2050-01-01") -> pd.DataFrame:
...
def get_dividends(ticker: str, asset_type: str) -> pd.DataFrame:
    # Placeholder – BaoStock also supports dividends if needed
    return pd.DataFrame()
