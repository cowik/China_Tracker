"""
Data fetching using yfinance with automatic suffix handling for A-shares.
Shanghai: 5/6开头 -> .SS, Shenzhen: 0/2/3开头 -> .SZ
"""
...
def _to_yfinance_ticker(ticker: str) -> str:
    """Convert clean ticker to yfinance format."""
...
def _retry_download(ticker_yf: str, start: str, end: str, retries=4, delay=2.0):
    """Download with retries and jitter to avoid rate limits."""
    for attempt in range(retries):
        try:
            df = yf.download(ticker_yf, start=start, end=end, progress=False, timeout=10)
            if df.empty:
                return pd.DataFrame()
            close = df['Close']
