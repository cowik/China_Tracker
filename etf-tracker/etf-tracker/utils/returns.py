...
def build_portfolio_index(
    ...
    # Remove duplicate index entries (keep the last value per date)
    combined = combined[~combined.index.duplicated(keep='last')]
    # Drop rows with invalid date (NaT)
    ...
    for h in holdings:
        ...
        if first_valid is not None:
            ...
            # SAFETY: force scalar in case loc returned a Series (due to duplicate index)
