...
# ----------------------------------------------------------- Strategy Panel --
st.divider()
st.subheader("📊 Strategy Holdings")
strategies = sheets_db.read_df("strategies_meta")
if strategies.empty:
    st.info("No strategies set up yet. Go to the Manage page to add them.")
else:
    for _, strat in strategies.iterrows():
        s_name = strat["strategy_name"]
        linked_etfs = str(strat.get("linked_etfs", "")).split(",")
        manual_tickers = str(strat.get("manual_tickers", "")).split(",")
        
        with st.expander(f"📁 {s_name}"):
            # 1. Fetch and display index constituents for linked ETFs
            watchlist_df = sheets_db.read_df("watchlist_etfs")
            etf_index_map = dict(zip(watchlist_df["ticker"], watchlist_df["index_code"]))
            
            all_etf_holdings = []
            for etf in linked_etfs:
                etf = etf.strip()
                if not etf:
                    continue
                idx_code = etf_index_map.get(etf, "")
                if not idx_code:
                    continue
                    
                # Check monthly cache
                cache_df = sheets_db.read_df("index_constituents_cache")
                cached = cache_df[cache_df["index_code"] == str(idx_code)] if not cache_df.empty else pd.DataFrame()
                needs_update = True
                if not cached.empty:
                    try:
                        last_date = pd.to_datetime(cached["as_of_date"].iloc[0], errors="coerce")
                        if pd.notna(last_date) and (pd.Timestamp.now() - last_date).days < 30:
                            needs_update = False
                    except:
                        pass
                
                if needs_update:
                    fresh_data = data_fetch.fetch_index_constituents(idx_code)
                    if fresh_data:
                        # Clear old cache for this index
                        cache_df = cache_df[cache_df["index_code"] != str(idx_code)] if not cache_df.empty else cache_df
                        new_rows = [{"index_code": str(idx_code), "stock_ticker": h["stock_ticker"], "stock_name": h["stock_name"], "weight": h["weight"], "as_of_date": pd.Timestamp.now().strftime("%Y-%m-%d")} for h in fresh_data]
                        combined_cache = pd.concat([cache_df, pd.DataFrame(new_rows)], ignore_index=True) if not cache_df.empty else pd.DataFrame(new_rows)
                        sheets_db.write_df("index_constituents_cache", combined_cache)
                        cached = combined_cache[combined_cache["index_code"] == str(idx_code)]
                
                if not cached.empty:
                    st.markdown(f"**ETF {etf} (Index: {idx_code}) - Top 10 Constituents:**")
                    st.dataframe(cached[["stock_ticker", "stock_name", "weight"]].rename(columns={"stock_ticker":"Ticker", "stock_name":"Name", "weight":"Weight (%)"}), use_container_width=True, hide_index=True)
                else:
                    st.warning(f"Could not load constituent data for ETF {etf} (Index {idx_code}).")

            # 2. Display Manual Tickers
            manuals = [t.strip() for t in manual_tickers if t.strip()]
            if manuals:
                st.markdown(f"**Manual Asset List:**")
                st.write(", ".join(manuals))
# --------------------------------------------------------------------------------
